...
function applySettings() {
  ...
  root.classList.toggle("ysf-hide-shorts", Boolean(settings.hideShorts));
  root.classList.toggle("ysf-subscription-videos-only", Boolean(settings.subscriptionVideosOnly));
  root.classList.toggle("ysf-four-across-subscriptions", Boolean(settings.fourAcrossSubscriptions));
  root.classList.toggle("ysf-dim-watched", Boolean(settings.dimWatched));
}
...
function isRegularVideoSection(section) {
  return (
    ...
    section.querySelector("ytd-video-renderer, ytd-grid-video-renderer")
  );
}
